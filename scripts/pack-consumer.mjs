...
try {
  ...
  await run(process.execPath, ['--input-type=module', '--eval', "import { buildMonth, createPointerInteraction, createScheduler, nativeDateAdapter } from 'headless-scheduler'; import { HeadlessScheduler } from 'headless-scheduler/react'; if (typeof createScheduler !== 'function' || typeof HeadlessScheduler !== 'function') process.exit(1); const scheduler = createScheduler({ timeZone: 'America/New_York', initialView: 'day', anchorDate: '2026-03-08T16:00:00.000Z' }); if (JSON.stringify(scheduler.getState().visibleRange) !== JSON.stringify({ start: '2026-03-08T05:00:00.000Z', end: '2026-03-09T04:00:00.000Z' })) process.exit(1); for (const [timeZone, firstDay] of [['Asia/Kolkata', '2026-02-28T18:30:00.000Z'], ['Pacific/Auckland', '2026-02-28T11:00:00.000Z']]) { const month = buildMonth({ month: '2026-03-15T12:00:00Z', events: [], adapter: nativeDateAdapter, timeZone, today: '2026-03-01T00:30:00Z' }); const days = month.weeks.flat(); const inside = days.filter(day => !day.outside); const today = days.find(day => day.today); if (month.key !== '2026-03' || inside.length !== 31 || inside[0]?.date !== firstDay || inside[0]?.dayNumber !== 1 || inside[30]?.dayNumber !== 31 || today?.dayNumber !== 1) process.exit(1) } for (const snapMinutes of [0, -15, NaN, Infinity]) { try { createPointerInteraction({ mode: 'move', start: '2026-08-27T09:00:00Z', end: '2026-08-27T10:00:00Z', pixelsPerMinute: 1, snapMinutes, onPreview() {}, onCommit() {} }); process.exit(1) } catch (error) { if (!(error instanceof RangeError)) process.exit(1) } }"], { cwd: consumer })
  ...
  console.log(JSON.stringify({ cleanPack: true, consumerImports: ['esm', 'cjs', 'react'], nativeTimezoneRegression: true, positiveOffsetMonthRegression: true, pointerValidationRegression: true, targets: expected.length }))
} finally {
